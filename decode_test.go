...
//
//	FITACTIVITY_REAL_FIT="$PWD/test_videos/activity.fit" go test github.com/wisborg/fitactivity
//
// Use an ABSOLUTE path: go test runs each package with its own directory as
// the working directory, so a repo-relative one // the fitactivity package and silently fails to open.
//
...
// else has. Same convention videofx uses for its own footage-gated probe tests.
...
// FITACTIVITY_REAL_FIT at a different activity will fail rather than pass -- this is a
...
// fitactivity/fittest's generated activities, which every other test in the tree
