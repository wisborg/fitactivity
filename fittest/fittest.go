...
// fitactivity.TestDecode_RealFile still pins Decode against a genuine Garmin
...
type Options struct {
	...
	// fitactivity.Track.At returns no sample there, exercising the
	...
	// it, and fitactivity.Decode sorts at decode time precisely so that later
	...
	// declare it.
	//
	// fitactivity.Decode resolves developer fields by NAME out of those
	...
	// fitactivity.Sample's HasPower/Power vs DevFields). the fitactivity package's
...
func Build(opts Options) ([]byte, error) {
	...
	for i := 0; i < opts.Count; i++ {
		...
		if inAnyPause(ts, opts.Start, opts.Pauses) {
			...
			// fitactivity.Track.At's own placeholder path (no sample) for a
